...
async Task GenerateMembers()
{
    ...
    // _supportUpdatedMemorySafetyRules and the per-required-type presence flags into a single ulong
    // _features so the table loop can short-circuit a polyfill with a single AND-compare against
    // entry.RequiredFeatures.
    const int FeatureBitExtensions = 0;
    const int FeatureBitUnsafe = 1;
    const int FeatureBitAllowsRefStruct = 2;
    const int FeatureBitUpdatedMemorySafetyRules = 3;
    var featureBitForType = new Dictionary<string, int>(StringComparer.Ordinal);
    for (var i = 0; i < requiredTypes.Length; i++)
    {
        featureBitForType[requiredTypes[i].TypeName] = 4 + i;
    }
    var totalFeatureBits = 4 + requiredTypes.Length;
    ...
    // The 'updated-memory-safety-rules' compiler feature (https://learn.microsoft.com/dotnet/csharp/language-reference/proposals/unsafe-evolution)
    // makes 'unsafe' members caller-unsafe and requires every 'extern' member to be explicitly marked
    // 'safe' or 'unsafe'. The 'safe' modifier itself needs C# preview, which the feature also requires,
    // so the define lets the polyfills emit it only where it is both needed and supported.
    sb.AppendLine($"    if (compilation.SyntaxTrees.FirstOrDefault()?.Options.Features.ContainsKey(\"updated-memory-safety-rules\") == true) features |= {1uL << FeatureBitUpdatedMemorySafetyRules}uL;");
    ...
    sb.AppendLine($"    if ((features & {1uL << FeatureBitUpdatedMemorySafetyRules}uL) != 0uL) prefixBuilder.Append(\"#define MEZIANTOU_POLYFILL_SUPPORT_UPDATED_MEMORY_SAFETY_RULES\\n\");");
    ...
    sb.AppendLine($"        new PolyfillFeature({1uL << FeatureBitUpdatedMemorySafetyRules}uL, \"UpdatedMemorySafetyRules\"),");
    foreach (var requiredType in requiredTypes)
    {
        var bitMask = 1uL << featureBitForType[requiredType.TypeName];
