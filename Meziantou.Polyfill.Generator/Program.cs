...
async Task GenerateMembers()
{
    ...
    // Instance method: only includes class names from enabled polyfills
    ...
    // Build a map: className -> { fieldName -> combinedBitMask }
    var classNameToFields = new Dictionary<string, Dictionary<string, ulong>>(StringComparer.Ordinal);
    foreach (var polyfill in polyfills)
    {
        foreach (var className in polyfill.PolyfillData.PolyfillExtensionsClassNames)
        {
            if (!classNameToFields.TryGetValue(className, out var fields))
            {
                fields = new Dictionary<string, ulong>(StringComparer.Ordinal);
                classNameToFields[className] = fields;
            }
            fields.TryGetValue(polyfill.CSharpFieldName, out var existingMask);
            fields[polyfill.CSharpFieldName] = existingMask | polyfill.CSharpFieldBitMask;
        }
    }

    foreach (var kvp in classNameToFields.OrderBy(x => x.Key, StringComparer.Ordinal))
    {
        var className = kvp.Key;
        var orderedFields = kvp.Value.OrderBy(x => x.Key, StringComparer.Ordinal);
        var condition = string.Join(" || ", orderedFields.Select(f => $"({f.Key} & 0x{f.Value:X}uL) != 0"));
