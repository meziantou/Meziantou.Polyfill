...
async Task GenerateMembers()
{
    ...
    sb.AppendLine("    // Pre-pass: collect which feature bits are needed by polyfills that match the");
    sb.AppendLine("    // options filter. Required-type polyfills whose bit appears here will be");
    sb.AppendLine("    // auto-included (as dependencies) even if they don't match the filter themselves.");
    sb.AppendLine("    var wantedFeatures = 0uL;");
    sb.AppendLine("    for (var i = 0; i < entries.Length; i++)");
    sb.AppendLine("    {");
    sb.AppendLine("        ref readonly var e = ref entries[i];");
    sb.AppendLine("        if (includeContext.Options is null || includeContext.Options.Include(e.DocId))");
    sb.AppendLine("            wantedFeatures |= e.RequiredFeatures;");
    sb.AppendLine("    }");
    ...
    sb.AppendLine("            {");
    sb.AppendLine("                if (!IsSymbolNeeded(includeContext, declared[j])) { allOk = false; break; }");
    sb.AppendLine("            }");
