...
var polyfills = assembly.GetManifestResourceNames()
      ...
      .Select((item, index) =>
      {
          var polyfillData = PolyfillData.Get(compilation, $$""""
                  // <auto-generated/>
          ...
          var typeName = polyfillData.XmlDocumentationId ?? Path.GetFileNameWithoutExtension(item).Replace(';', ':');
          ...
          if (symbols.Length > 1)
          {
              throw new InvalidOperationException($"Multiple symbols found for '{typeName}' (resource name: {item}): {string.Join(", ", symbols.Select(s => s.ToDisplayString()))}");
          }
