...
{
    /// <summary>
    /// Asynchronously loads an <see cref="XElement"/> from a <see cref="Stream"/>.
    /// </summary>
    ...
    extension(XElement)
    {
        ...
        async
#endif
        Task<XElement> LoadAsync(Stream stream, LoadOptions options, CancellationToken cancellationToken)
        {
            if (stream is null)
